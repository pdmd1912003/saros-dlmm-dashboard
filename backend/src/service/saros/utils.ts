import { BinInfo, DLMMPosition, PoolInfo, PositionAnalytics } from './types'
...
export function decodeBNArray(arr: any[]): number[] {
  return arr.map((v) => {
    try {
      return typeof v === "object" && "toString" in v
        ? Number(BigInt(v.toString()))
        : Number(v);
    } catch {
      return 0;
    }
...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (err.message?.includes("429")) {
        console.warn(`⚠️ RPC rate limit hit (429). Retry attempt ${attempt}/${retries}...`);
        if (attempt < retries) {
  ...
  throw new Error(`Max retry attempts (${retries}) reached`);
}
